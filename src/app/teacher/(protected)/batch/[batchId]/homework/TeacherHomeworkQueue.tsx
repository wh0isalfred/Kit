...
import {
  returnTeacherHomework,
  getTeacherSubmissionFileUrl,
  type GradingQueueItem,
  type HomeworkAssignment,
} from "../actions";
import TeacherByAssignmentView from "./TeacherByAssignmentView";

/**
 * Direct port of admin's HomeworkQueue.tsx — the FIFO skip/restore
 * logic, stale-submission highlighting, and image-preview-vs-
 * download-button branching are correct, proven mechanics that don't
 * change meaning based on who's grading. Only the action imports
 * differ (teacher-gated versions from ../actions, which themselves
 * call the now-teacher-aware RPCs from 0046) and every call now
 * threads batchId through, since the teacher actions need it to
 * verify access — admin's versions didn't need this because
 * assertAdmin() alone was sufficient.
 */
...
export default function TeacherHomeworkQueue({
  batchId,
  initialQueue,
  initialError,
  assignments,
  ...
  return (
    <div className="hw-queue-tab">
      ...
      {view === "queue" ? (
        <QueueView batchId={batchId} queue={queue} setQueue={setQueue} />
      ) : (
        <TeacherByAssignmentView batchId={batchId} assignments={assignments} />
      )}
    </div>
  );
}
...
function QueueCard({
  ...
  useEffect(() => {
    if (isImage && item.storage_path) {
      (async () => {
        setFileLoading(true);
        const res = await getTeacherSubmissionFileUrl(batchId, item.storage_path!);
  ...
  async function onReturn() {
    ...
    const res = await returnTeacherHomework(batchId, item.submission_id, feedback);
    if (!res.ok) {
      setBusy(false);
      setError(res.error);
      onRestore();
